<div>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
        @livewire('dashboard.card', ['title' => 'Students', 'count' => $students_count])
                ...
                <tbody>
                    @foreach ($pending_payments as $pending_payment)
                        <tr class="odd:bg-gray-100 even:bg-gray-50 hover:bg-gray-200 transition">
                            <td class="py-2 px-4 border border-gray-300 text-center">{{ $pending_payment->student_id }}
                            </td>
                            <td class="py-2 px-4 border border-gray-300 text-center">
                                {{ $pending_payment->student->firstname }}</td>
                            <td class="py-2 px-4 border border-gray-300 text-center">
                                {{ $pending_payment->student->lastname }}</td>
                            <td class="py-2 px-4 border border-gray-300 text-center">
                                {{ $pending_payment->student->course }}</td>
                            <td class="py-2 px-4 border border-gray-300 text-center">
                                {{ $pending_payment->student->phone }}</td>
    ...
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold text-blue-800 mb-4 text-center">New Admissions</h1>
            ...
            <table class="w-full border-collapse border border-gray-300">
                ...
                <tbody>
                    @foreach ($new_students as $new_student)
                        <tr class="odd:bg-gray-100 even:bg-gray-50 hover:bg-gray-200 transition">
                            <td class="py-2 px-4 border border-gray-300 text-center">{{ $new_student->id }}</td>
                            <td class="py-2 px-4 border border-gray-300 text-center">{{ $new_student->firstname }}</td>
